    ...
    : AudioProcessorEditor(ownFilter)
{
	...
	Compressor1->setSkewFactorFromMidPoint(-20);
    ...
    addAndMakeVisible (BassBoost = new ToggleButton ("new toggle button"));
    BassBoost->setButtonText (TRANS("Compressor"));
...
void DiddyBop_AudioProcessorEditor::resized()
{
    ...
    BassBoost->setBounds (232, 152, 150, 24);
...
void DiddyBop_AudioProcessorEditor::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == BassBoost)
    {
        //[UserButtonCode_BassBoost] -- add your button handler code here..
		getProcessor()->bassBoost = BassBoost->getToggleState();
        //[/UserButtonCode_BassBoost]
    }
...
<JUCER_COMPONENT documentType="Component" className="DiddyBop_AudioProcessorEditor"
  ...
  <SLIDER name="Centre Frequecy" id="5d1d6c356b59ff42" memberName="OutPut_Gain"
          ...
          needsCallback="1"/>
  <TOGGLEBUTTON name="new toggle button" id="3562c0526bb19b3f" memberName="BassBoost"
                virtualName="" explicitFocusOrder="0" pos="232 152 150 24" buttonText="Compressor"
                connectedEdges="0" needsCallback="1" radioGroupId="0" state="0"/>
</JUCER_COMPONENT>
